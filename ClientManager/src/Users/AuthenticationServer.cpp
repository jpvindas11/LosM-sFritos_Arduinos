...
bool AuthenticationServer::hashPassword(const std::string& password, const unsigned char* salt, unsigned char* hash) {
  // Usar Argon2id con salt personalizado
  if (crypto_pwhash(hash, 32, password.c_str(), password.length(), salt, crypto_pwhash_OPSLIMIT_INTERACTIVE,
...
bool AuthenticationServer::verifyPassword(const std::string& password, const std::string& storedHash, const std::string& storedSalt) {
...
bool AuthenticationServer::parseMessage(const std::string& message, std::string& command,
...
std::string AuthenticationServer::processLogin(const std::string& username, const std::string& password) {
...
bool AuthenticationServer::registerUser(const std::string& username, const std::string& password) {
  std::lock_guard<std::mutex> lock(usersMutex);
  ...
  if (password.length() < 4) {
    std::cout << "Error: Contraseña muy corta para usuario '" << username << "' (mínimo 4 caracteres)" << std::endl;
    return false;
  }
  ...
  unsigned char hash[32];
  if (!hashPassword(password, salt, hash)) {
    std::cout << "Error: No se pudo hashear la contraseña para usuario '" << username << "'" << std::endl;
    return false;
  }
  ...
  newUser.salt = std::string(reinterpret_cast<char*>(salt), crypto_pwhash_SALTBYTES);
  newUser.isConnected = false;

  users[username] = newUser;
  
  std::cout << "Usuario '" << username << "' registrado exitosamente por el servidor" << std::endl;
  return true;
}
...
std::string AuthenticationServer::getUserSaltHex(const std::string& username) {
  ...
  std::stringstream ss;
  for (size_t i = 0; i < it->second.salt.length() && i < crypto_pwhash_SALTBYTES; ++i) {
