...
public:
    ...
    ServerDiscoveryClient(int udp_port = 14999) 
...
// Ejemplo de uso:
int main() {
    ServerDiscoveryClient client;
    
    // Buscar todos los servidores
    std::cout << "\n=== Buscando todos los servidores ===" << std::endl;
    auto servers = client.discoverServers(3);
    
    if (servers.empty()) {
        std::cout << "No se encontraron servidores" << std::endl;
        return 0;
    }
    
    std::cout << "\nServidores encontrados: " << servers.size() << std::endl;
    for (size_t i = 0; i < servers.size(); i++) {
        std::cout << i+1 << ". " << servers[i].name 
                  << " - " << servers[i].ip << ":" << servers[i].port 
                  << " (Tipo: " << (int)servers[i].serverType << ")" << std::endl;
    }
    
    // Conectar al primer servidor usando TCP
    if (!servers.empty()) {
        std::cout << "\nConectando al servidor: " << servers[0].name << std::endl;
        
        Socket tcpSocket;
        if (tcpSocket.create() && 
            tcpSocket.connectToServer(servers[0].ip, servers[0].port)) {
            std::cout << "Conectado exitosamente!" << std::endl;
            
            // Aquí puedes usar el socket TCP normalmente
            // tcpSocket.bSendData(...);
            // tcpSocket.bReceiveData(...);
